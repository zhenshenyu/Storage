...
def gradAscent(dataMatIn, classLabels):
    ...
    # weights 代表回归系数， 此处的 np.ones((n,1)) 创建一个长度和特征数相同的矩阵，其中的数全部都是 1
    weights = np.ones((n, 1))
    h = np.zeros((m, 1))
    for k in range(maxCycles):  # heavy on matrix operations
        pos = 0
        for i in np.nditer(dataMatrix * weights):
            h[pos][0] = 1.0 / (1 + exp(-i))
            pos += 1
        print 'h = ', h.transpose()
        # labelMat是实际值
...
def simpleTest(location):
    dataMat, labelMat = loadDataSet(location)
...
def colicTest():
    ...
    Returns:
        errorRate -- 分类错误率
    """
    frTrain = open('input/HorseColicTraining.txt')
    frTest = open('input/HorseColicTest.txt')
    trainingSet = []
    trainingLabels = []
